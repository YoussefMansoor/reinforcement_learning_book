...
5. **The exploration problem (§4.6) and its three solutions (§4.7–4.10):** Exploring Starts, on-policy ε-greedy control, and off-policy control with importance sampling.
...
*Figure 4.1: The contrast at a glance. **Dynamic Programming (left)** needs the entire model — every transition probability p(s′ | s, a) and reward must be known up front, so the whole graph is "lit up." **Monte Carlo (right)** ignores the model entirely: it simply follows one episode at a time (the red path), observes the rewards that actually occur, and learns by averaging those observed returns. The faded edges underscore that MC never needs to know the transitions it didn't take.*
...
where Gₜ is the **return** from time t onwards:

```text
Gₜ = Rₜ₊₁ + γ·Rₜ₊₂ + γ²·Rₜ₊₃ + ⋯
```

Monte Carlo prediction estimates this expectation by a beautifully simple mechanism: **run episodes, observe the returns Gₜ, and average them**. By the Law of Large Numbers, as we collect more and more sampled returns for a given state, their average converges to the true expected value vπ(s).
...
```text
S₀, A₀, R₁, S₁, A₁, R₂, S₂, …, S_{T-1}, A_{T-1}, R_T, S_T
```

where S_T is the **terminal state** and T is the **terminal time step**.

The definition of Gₜ above is a *forward* sum, but computing it that way for every t would be wasteful — we'd redo overlapping work. Instead, notice that the return obeys a simple recursive relationship:

```text
Gₜ = Rₜ₊₁ + γ·Gₜ₊₁
```

In words: *the return at time t is the immediate reward plus the discounted return from the next step.* This lets us compute every Gₜ in a single **backward sweep**, starting from the end of the episode (where G_T = 0, because nothing follows the terminal state) and walking back to the start:
...
**A concrete example.** Suppose an episode lasts T = 3 steps with rewards R₁ = 2, R₂ = 0, R₃ = 5, and discount γ = 0.9. Sweeping backward:

```text
G₃ = 0
G₂ = R₃ + γ·G₃ = 5 + 0.9·(0)   = 5
G₁ = R₂ + γ·G₂ = 0 + 0.9·(5)   = 4.5
G₀ = R₁ + γ·G₁ = 2 + 0.9·(4.5) = 6.05
```

So the observed return from the start state is G₀ = 6.05. This single backward pass gives us the return from *every* time step at once — which is exactly what the Monte Carlo algorithm needs.

### 4.2.3 First-Visit vs. Every-Visit MC

Here is a subtle question: what if the agent visits the same state s **more than once** in a single episode? How many of those visits should contribute to our estimate of vπ(s)?
...
**A worked example.** Take a single episode that visits state s twice. Let the trajectory of states be s → x → s → (terminal), with rewards R₁ = 1, R₂ = 2, R₃ = 3 and γ = 1 for simplicity. State s appears at time steps t = 0 and t = 2.
...
- The **first visit** is at t = 0, with return G₀ = 6.
- The **second visit** is at t = 2, with return G₂ = 3.

The two methods treat these differently:

- **First-Visit MC** records only G₀ = 6 for this episode (it ignores the later visit).
- **Every-Visit MC** records *both* G₀ = 6 and G₂ = 3.

Across many episodes, each method simply averages all the returns it has recorded for s. For instance, if three separate episodes gave first-visit returns of 6, 4, and 8, then the estimate is (6 + 4 + 8) / 3 = 6.

Both methods converge to the true value vπ(s) as the number of recorded returns grows. **First-Visit MC is unbiased.** Every-Visit MC is biased for finite samples but also converges asymptotically — and it is often preferred in practice because it extends more naturally to function-approximation settings (Chapter 6).
...
*Figure 4.2: The Racetrack task. The car (green dot) begins on the start line with zero velocity and must reach the finish line (red) in as few steps as possible while staying on the track (light cells) and avoiding the walls (dark cells). Its **state** is its grid position together with its current velocity (vₓ, v_y); each **action** nudges that velocity. Monte Carlo learns a fast racing line purely by driving the car many times and averaging the returns — it never needs to know the car's physics.*

- **State:** Current grid position + current velocity (horizontal vₓ, vertical v_y).
    - Velocity components: non-negative, each < 5, not both zero (except at the start).
- **Actions:** Increment or decrement each velocity component by ±1, or leave it unchanged → **9 possible actions**.
...
```text
π′(s) = argmaxₐ Σ_{s′, r}  p(s′, r | s, a) · [ r + γ·V(s′) ]
```

which depends on p(s′, r | s, a) — the very transition probabilities we don't have.
...
This alternation — episode by episode — converges toward the optimal action-value function Q\* and the optimal policy π\*.
...
**ε-greedy policy:** Most of the time (with probability 1 − ε) take the greedy action, but with probability ε take a *random* action. This ensures every action is tried occasionally. Formally, writing |A(s)| for the number of actions available in state s:

```text
              ⎧  1 − ε + ε / |A(s)|   if a = argmaxₐ′ Q(s, a′)   (the greedy action)
   π(a | s) = ⎨
              ⎩  ε / |A(s)|           otherwise                  (any other action)
```
...
The full control loop uses this ε-greedy policy in both roles — generating the data and being improved:
...
The data we have comes from policy b, but we want to estimate values under policy π. The returns we observe therefore have the *wrong* expectation — they reflect b, not π.

**Importance Sampling** is a general statistical technique for estimating an expectation under one distribution (π) using samples drawn from another (b). The trick is to weight each return by how much more (or less) likely its trajectory was under π compared to b.
...
Here is the beautiful cancellation: the full trajectory probabilities *also* depend on the environment's transition probabilities p(s′ | s, a) — but these appear identically in the numerator and denominator, so they cancel. The ratio depends only on the two policies, never on the unknown MDP dynamics.

**Intuition.** Reading the ratio ρ:

- ρ > 1  →  trajectory **more** likely under π than b  →  weight it **up**.
- ρ < 1  →  trajectory **less** likely under π than b  →  weight it **down**.
- ρ = 0  →  trajectory could *never* happen under π  →  **discard it**.

### 4.9.2 Ordinary vs. Weighted Importance Sampling

Two variants exist, with a classic bias-variance tradeoff. Let T(s) denote the set of time steps at which state s is visited (across all episodes), and let N(s) = |T(s)| be how many such visits there are.

**Ordinary Importance Sampling** divides the weighted returns by the *count* of visits:

```text
            Σ over t in T(s)  of  ρ(t : end) · Gₜ
   V(s) = ───────────────────────────────────────
                          N(s)
```
...
```text
            Σ over t in T(s)  of  ρ(t : end) · Gₜ
   V(s) = ───────────────────────────────────────
            Σ over t in T(s)  of  ρ(t : end)
```

- Biased initially (but the bias → 0 asymptotically).
...
Let Cₙ be the cumulative sum of importance weights for a state after n returns, and Vₙ the estimate after n returns. Then:
...
Here is the complete off-policy MC control algorithm. It estimates the optimal policy π\* using a soft behavior policy b:
...
The target policy π always chooses `left`, giving true value vπ(s) = 1. The behavior policy b chooses `right` or `left` with equal probability (0.5 each).
...
In Dynamic Programming, the estimate for state s depends on the estimates for its successor states s′. An error in one state propagates to its neighbors through the Bellman equation. This is called **bootstrapping**.

Monte Carlo does **not bootstrap**. Each return Gₜ is computed entirely from real rewards observed in the episode. The estimate of V(s) does not depend on the estimate of any other V(s′).
...
| **On-Policy (ε-greedy)** | Built into the policy | Best ε-soft policy | Cannot learn the fully greedy optimum |
